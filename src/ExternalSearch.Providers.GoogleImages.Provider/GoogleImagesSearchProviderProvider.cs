...
using CluedIn.ExternalSearch.Providers.GoogleImages;
...
namespace CluedIn.Provider.ExternalSearch.GoogleImages
{
    public class GoogleImagesSearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        ...
        public GoogleImagesSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<CrawlJobData> GetCrawlJobData(ProviderUpdateContext context, IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var result = new GoogleImagesExternalSearchJobData(configuration);
        ...
        {
            if (jobData is GoogleImagesExternalSearchJobData result)
            {
                return await Task.FromResult(result.ToDictionary());
            }

            throw new InvalidOperationException($"Unexpected data type for {nameof(GoogleImagesExternalSearchJobData)}, {jobData.GetType()}");
        }
